          ...
          @for (bookmark of bookmarks(); track bookmark.id) {
            <div class="group relative">
              <a
                ...
                class="flex flex-col items-center justify-center gap-4 p-6 rounded-[2rem] group-hover:scale-105 group-hover:-translate-y-1 transition-all duration-300"
                [class]="glassEffect() + ' ' + itemHover()"
              >
                <div class="w-16 h-16 rounded-2xl flex items-center justify-center p-3 shadow-inner bg-white/5">
                  <img [src]="bookmark.icon" alt="" class="w-full h-full object-contain filter drop-shadow-md" />
                </div>
                <span class="text-sm font-medium opacity-80 group-hover:opacity-100 transition-opacity truncate w-full text-center">
                  {{ bookmark.title }}
                </span>
              </a>
...
export class App implements OnInit {
  ...
  handleBookmarkSubmit(e: Event) {
    ...
    if (editItem) {
      this.bookmarks.update(prev => prev.map(b => b.id === editItem.id ? { ...b, title, url, icon } : b));
    } else {
      this.bookmarks.update(prev => [...prev, { id: Date.now().toString(), title, url, icon }]);
    }
    this.closeModals();
  }
}
